...
function DataCenter() {
	...
	//初始化dataCenter对象
    (function (base) {
        var temp, result, obj, item, func;
        result = new DataCenter();

        for (var type in base) {

            // 每个接口种类都重新实例对象 然后分别赋值
            obj = result[type] = new Object();
            temp = base[type]; //获取api分类

            for (var key in temp) {
                item = temp[key]; //api配置项

                //判断是否为枚举
                if (key != "enumeration"){
                    if (item.url.indexOf("http") == -1) {
                        item.url = root.protocol + "://" + baseConfig.service.use + (item.baseRoot || root.default) + item.url;
                    }

                    //真正赋予数据能力
                    obj[key] = new DataCenterBase(item, type + "." + key + "." + "event");
                }else{
                    obj[key] = item;
                }
            }
    ...
    function DataCenter() {
        ...
        this.dataCenterBase = DataCenterBase;
    }
};
